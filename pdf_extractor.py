    ...
    def _perform_api_extraction(self, pdf_path: Path, filename: str, page_range: str) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                try:
                    retry_system = "経理補助AI。複式簿記原則：1取引=借方+貸方、金額合計一致必須。出力はJSON配列のみ。各要素は5カラム（伝票日付/借貸区分/科目名/金額/摘要）。余計な文字・コードフェンス禁止。"
...
【形式例】
[
  {"伝票日付":"2025/2/14","借貸区分":"借方","科目名":"退居敷金","金額":55000,"摘要":"ルベール鷺宮102号 白坂安純様 敷金返還精算"},
  {"伝票日付":"2025/2/14","借貸区分":"貸方","科目名":"現金（西京普通）","金額":55000,"摘要":"ルベール鷺宮102号 白坂安純様 敷金返還精算"}
]
...
- 抽出不能時も1要素返す"""
    ...
    def _get_standard_system_prompt(self) -> str:
        ...
        return """あなたは経理補助AIです。
以下の伝票画像（OCR認識済テキスト）をもとに、
日本の「複式簿記（Double-entry bookkeeping）」原則に従って仕訳を構造化してください。

★★重要★★
【複式簿記原則の遵守】
- 各伝票は「1取引＝借方1本以上＋貸方1本以上」の複式簿記構造として出力する。
- 借方・貸方の合計金額は必ず一致させること。
- 単一側（借方のみ／貸方のみ）の出力は禁止。
- 同一取引の借方・貸方は、それぞれ別のオブジェクトとして出力する。
- 複数行（例：借方2行＋貸方1行）の場合、共通の摘要・日付を共有させる。
- 出力順は「借方 → 貸方」。
...
2) "借貸区分"（"借方" または "貸方"）
3) "科目名"（日本語で整形。例：「現金（西京普通）」）
4) "金額"（半角数字、正整数）
5) "摘要"（物件名・契約者・備考を含む共通摘要）

【出力例】

[
  {"伝票日付":"2025/2/14","借貸区分":"借方","科目名":"退居敷金","金額":55000,"摘要":"ルベール鷺宮102号 白坂安純様 敷金返還精算"},
  {"伝票日付":"2025/2/14","借貸区分":"借方","科目名":"預り金","金額":41549,"摘要":"ルベール鷺宮102号 白坂安純様 敷金返還精算"},
  {"伝票日付":"2025/2/14","借貸区分":"貸方","科目名":"現金（西京普通）","金額":27249,"摘要":"ルベール鷺宮102号 白坂安純様 敷金返還精算"},
  {"伝票日付":"2025/2/14","借貸区分":"貸方","科目名":"売上（退去営業費）","金額":69300,"摘要":"ルベール鷺宮102号 白坂安純様 敷金返還精算"}
]
...
- 枠ごとの借方金額合計と貸方金額合計は一致させる。合わないJSONを出力してはならない。
  不足分は"不明"レッグを追加して合わせる（科目名は空文字または"不明"で可）。
...
【注意事項】
- OCR結果に曖昧さがある場合は、摘要末尾に【OCR注意】を付記。
- 科目名は日本語で整形（例：「普通預金」ではなく「現金（西京普通）」）。
- 日付・金額は半角数字で統一。
- 金額に負数や記号は使わない。
- 抽出不能でも配列1要素は必ず返す。"""
